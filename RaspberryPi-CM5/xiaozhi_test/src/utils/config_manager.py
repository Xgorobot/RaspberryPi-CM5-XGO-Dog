...
class ConfigManager:
    ...
    if la=="cn":
        CONFIG_FILE = CONFIG_DIR / "config_cn.json"
    else:
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        try:
            # 先尝试从当前工作目录加载
            if la=="cn":
                config_file = Path("config/config_cn.json")
            else:
                config_file = Path("config/config_en.json")          
            ...
            if getattr(sys, 'frozen', False) and hasattr(sys, '_MEIPASS'):
                if la=="cn":
                    config_file = Path(sys._MEIPASS) / "config" / "config_cn.json"
                else:
                    config_file = Path(sys._MEIPASS) / "config" / "config_en.json"              
                if config_file.exists():
                    config = json.loads(
